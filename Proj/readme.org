...
*** Qt [0/3]
    ...
    :END:
1) [ ] Start a QWidget application on Qt
2) [ ] Use the build system to externally to obtain a makefile [0/2]
   1) [ ] =qmake=
       #+BEGIN_SRC bash
       cd /path/to/project
       qmake -project # if the project is not yet created, otherwise
       ...
       #+END_SRC
   2) [ ] =cmake=
3) [ ] Generate =compile_commands.json= for LSP integration [0/2]
   1) [ ] Cmake-based projects: If your project builds with CMake, it can
      generate this file. You should enable it with:
     #+BEGIN_SRC bash
     cmake -DCMAKE_EXPORT_COMPILE_COMMANDS=1
     #+END_SRC
     compile_commands.json will be written to your build directory. 
     You should symlink it (or simply copy it) to the root of your
     source tree, if they are different.
   2) [ ] other build systems, using =bear=: For a make-based build, you can 
      1) =make clean=;
      2) =bear -- make=: to generate the file (and run a clean build!).
