...
12) [-] Create thread for fragranceDiffuser
    ...
    4) [ ] if =cur_ad.enabled()=
       1) [ ] Get fragrance from it
       2) [ ] Look in FragranceManager for its settings
       3) [ ] Assign fragrance to the FragranceDiffuser
       4) [ ] Get On and Off times from fragrances
       5) [ ] Load timer
       6) [ ] When timer expires, reload with the other value
