...
3) [ ] Design the UI
   1) [ ] Draw the several UI views
      1) [ ] =File->New file->Qt->Qt Designer form class->Widget= and name it
	 1) [ ] Normal mode view: =MainWindow=
	 2) [ ] Interaction mode view: =IntWindow=
	 3) [ ] Image filtering view: =ImgFiltWindow=
	 4) [ ] GIF view: =GIFWindow=
	 5) [ ] Sharing mode: =SharWindow=
   2) [ ] Use dummy pushbuttons to create/destroy the several UI views
