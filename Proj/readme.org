...
6) [ ] Implement Twitter sharing
   1) [ ] Set focus to message when transitioning to Edit Post
   2) [ ] Define a default message in case the user does not enter any message
   3) [ ] Get coordinates of VirtualKeyboard to map gesture to it
...
8) [ ] Create GIF: Interaction mode
9) [ ] Reproduce video with audio
10) [ ] Implement client-server architecture
