...
-- Found ImageMagick: /usr/local/lib/libMagick++-7.Q16HDRI.so (found version "7.1.0-23") 
CMAKE INCLUDE DIRS: /usr/local/include/ImageMagick-7
CMAKE LIBS DIRS: /usr/local/lib/libMagick++-7.Q16HDRI.so
-- Configuring done
-- Generating done
-- Build files have been written to: /home/zmpl/OneDrive-UM/Univ/MI_Electro/Sem7/SEC/2021-22/r
epo/Proj/code/LS/LSApp/build
#+END_EXAMPLE

#+BEGIN_EXAMPLE
MagickConfig: /home/zmpl/OneDrive-UM/Univ/MI_Electro/Sem7/SEC/2021-22/Buildroot/buildroot-2021
.08.1/output/host/bin/Magick++-config                                                        
LDFlags: -L/home/zmpl/OneDrive-UM/Univ/MI_Electro/Sem7/SEC/2021-22/Buildroot/buildroot-2021.08
.1/output/host/bin/../aarch64-buildroot-linux-gnu/sysroot/usr/lib -lMagick++-7.Q16HDRI -lMagic
kWand-7.Q16HDRI -lMagickCore-7.Q16HDRI                                                       
...
7) Play and stop video
   ...
   m_mediaPlayer->play();
   #+END_SRC
* ✔ DONE Synchronization Class                                     :noexport:
  :LOGBOOK:
  - State "✔ DONE"     from              [2022-02-04 sex 15:50]
  - Validated: see [[file:code/pthreads/condVar/main.cpp][here]] 
  :END:
...
10) [X] Create a timer to trigger periodically (every hour) and the respective
    callback
    1) Normal mode
       1) Create a callback
	  1) Get current time
	  ...
	  2) =if cur_ad.enabled()=
		...
		4) Go to 2)
	  3) else, do nothing
       3) [X] Handle diffuser for fragranceDiffuser
	  1) [X] Create Fragrance class
	  2) [X] Create Fragrance Manager class
	  3) [X] Create Fragrance Diffuser class
	  4) [X] if =cur_ad.enabled()=
	  5) [X] Get fragrance from it
	  6) [X] Look in FragranceManager for its settings
	  7) [X] Assign fragrance to the FragranceDiffuser
	  8) [X] Get On and Off times from fragrances
	  9) [X] Load timer
	  10) [X] When timer expires, reload with the other value
    2) Check for interaction mode
       1) If user detected AND (=mode == Welcome OR mode == NORMAL=)
	  1) OnInterMode_pressed();
	  2) Connect message queue and process
11) [X] Implement client-server architecture
    1) [X] Connect to server: bool connectToRemote()
    2) [X] Create thread to receive frames
    3) [X] Create thread to process frames
    4) [X] Deploy a server to test the logic
12) [X] Create thread to download files
*** Frames
1) Ad: "A"
2) Tamanho do buffer daqui para a frente
3) nome do ficheiro
4) link
5) fragrance_id
6) filter_id
...
1) Ze
   1) SW 
      1) Remote client + DB server deployment
	 1) Download Hugo code
	 2) Compile
      2) Run RC + LS in localhost
   2) Rasp
      1. Compile example program to rasp
2) Hugo
   1) buildroot
   2) Compile example program to rasp
* Deployment
** Buildroot
 - What is?
 - Why use it?: to generate custom embedded Linux systems according to our
   application's requirements
- [[file:~/OneDrive-UM/Univ/MI_Electro/Sem7/SEC/2021-22/Buildroot/buildroot-2021.08.1][Path]]
*** Installation
    1. Check the manual
    2. Install mandatory packages: =sudo apt install <package>=
    3. Install optional packages:
       1. =sudo apt install ncurses5=
       2. Source control version: =sudo apt install git=
    4. Verify installed packages: =which <package>=
    5. Create a directory in home directory to put all buildroot files
    6. Download the buildroot installation
*** Compilation
1) Setup:
     #+begin_src bash
     # help
...
#   make O=dir             - Locate all output files in "dir", including .config
      ...
      # obtain the list of available default configurations (for devices)
      make list-defconfigs | grep raspberry
      # create the default config for raspberry pi 64
        make raspberrypi_64_defconfig
      #+end_src
2) Run configuration: see [[id:02eb8d33-4f65-49c4-b33d-e0f4b51acb87][here]]
3) Run =make= to build the image
   1) The image should be built incrementally
      1) Qt support
      2) Opencv
      3) Camera support
...
*** Configuration
    :PROPERTIES:
    :ID:       02eb8d33-4f65-49c4-b33d-e0f4b51acb87
    :END:
       1. =make menuconfig=
       2. Under system configuration:
	  1. go to "run a getty (login prompt) after boot"
	  2. change the "TTY port" content to "ttyAMA0" and the baudrate to 115200
       3. Select "root password"
       4. Go to Target Packages->Networking applications and enable:
	  1. dhcpcd
	  2. droapbear
       5. Go to the first window and enter in "filesystem images", navifate to "tar
	  the root filesystem" and enable it, after it's enabled you must select a
	  "compression method" =bzip2=
       6. Apply all changes to the image: =make=
*** Deployment
- Flashing SD card
  1) Option B: use =dd= command
     #+BEGIN_SRC bash
     sudo dd if=output/images/sdcard.img of=/dev/sda status=progress
     #+END_SRC
- Configuring system on Raspberry Pi
  - edit cmdline.txt: replace =ttyAMA0= with =ttyS0=
  - edit config.txt: =enable_uart=1==
*** Check boot process
    1. Check UART:
    2. Use a terminal console to connect
    3. Test the Ethernet and ssh connection
    4. Secure copy to transfer files
       1. and also can be used to configure and perform REMOTE debugging
          sessions
*** Check buildroot Examples
    - Compilation: =./output/host/bin/arm-linux-gcc-10.3.0=
    - Transfer: =scp <src-path> username@IP:/<dest-path>=

**** Cross compilation
1) Keep the CMakeLists.txt at the original path
2) Create different build folders inside =./=
   1) =build_desktop=
   2) =build_rasp=
3) Create a toolchain file ([[file:code/LS/LSApp/toolchain/rasptoolchain.cmake::#File%20raspberrytoolchain.cmake%20for%20ROS%20and%20system%20packages%20to%20cross%20compile.][src]])
4) Add the toolchain path to =CMakeLists.txt=

* Build Qt with CMake for Embedded Systems

#+BEGIN_SRC bash
# 4m:51s https://www.youtube.com/watch?v=Iz0V0WPUArY
vim setup-env-talk

env_setup_file = path/to/sdk/environment-setup-aarch64-tdx-linux

qtc_dir = /home/burkhard/Qt/Qt5.15.2/Tools/QtCreater

ssh root@<device_name>.local

configure-qtcreator.sh --name Goldfinger --env-setup $env_setup_file --creator-base $qtc_dir --device <device_name>.local
#+END_SRC
...
** ✔ DONE Copy files
   :LOGBOOK:
   - State "✔ DONE"     from "☛ TODO"     [2022-02-15 ter 04:41]
   - State "☛ TODO"     from              [2022-02-15 ter 04:01]
   :END:
1) Daemon
2) Device drivers
   1) Frag
   2) USS
3) LSApp and directories

#+BEGIN_SRC bash
scp LSApp/ root@10.42.0.100:/etc/LS
...
** ✔ DONE Config.txt
   :LOGBOOK:
   - State "✔ DONE"     from "☛ TODO"     [2022-02-15 ter 04:41]
   - State "☛ TODO"     from              [2022-02-15 ter 04:15]
   :END:
- Rotate screen
  #+BEGIN_EXAMPLE
  # rotate display
...
** ✔ DONE Init script
   :LOGBOOK:
   - State "✔ DONE"     from "☛ TODO"     [2022-02-15 ter 04:10]
   - State "☛ TODO"     from              [2022-02-15 ter 04:02]
   :END:
...
** Fix application [0/4]
2) [ ] Fix Window size / scaling
1) [ ] Fix paths
3) [ ] Fix logic for mqueues
4) [ ] Fix User detection timing
