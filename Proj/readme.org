...
#+BEGIN_SRC plantuml :file diags/plantuml/seq-diag/output/interaction-mode.png
  @startuml
  ' ---------- SETUP ----------------
  ' strict uml style and hide footboxes
  skinparam style strictuml
...
== Activate camera feed ==
  ...
  par
      loop while (user in range \n&& ! user_timeout)
          LS -> UIE: grab frame from camera \nand display it on window
	  activate UIE
...
#+RESULTS:
[[file:diags/plantuml/seq-diag/output/interaction-mode.png]]
