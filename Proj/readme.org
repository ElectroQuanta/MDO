...
10) [ ] Create thread to check if normalMode must run
    ...
    2) Create a callback
	  ...
	  1) =cur_ad = ad=
	  2) =cur_ad.enable()=
11) [X] Create thread for videoManager
    1) while(1)
       1) =if cur_ad.enabled()=
	  ...
	  2) For each =media_file_url=
		...
		1) if !cur_ad_enabled() || appmode
	     4) Go to 2)
       2) else, do nothing
12) [X] Create thread for fragranceDiffuser
    1) [X] Create Fragrance class
    2) [X] Create Fragrance Manager class
    3) [X] Create Fragrance Diffuser class
    4) [X] if =cur_ad.enabled()=
       1) [X] Get fragrance from it
       2) [X] Look in FragranceManager for its settings
       3) [X] Assign fragrance to the FragranceDiffuser
       4) [X] Get On and Off times from fragrances
       5) [X] Load timer
       6) [X] When timer expires, reload with the other value
13) [ ] Create thread to download files
14) [ ] Implement client-server architecture
    1) [ ] Create thread to receive frames
    2) [ ] Create thread to process frames
*** Frames
1) Ad
