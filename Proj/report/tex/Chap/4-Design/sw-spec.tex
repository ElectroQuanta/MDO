...
\item\emph{\texttt{Parser}}: the parser has only one thread that is its execution. This thread has high priority because it is mandatory to parse and execute the commands that are received, as soon as they are received.
...
\begin{figure}[htb!]
\centering
    \includegraphics[width=1\columnwidth]{./img/flow-rs-recv.png}
...
Then, by the structure rxFrame it is verified if the data is a command or a database query. If it is a command, the command is processed and operated and if it is a database query, it queries the command. 
