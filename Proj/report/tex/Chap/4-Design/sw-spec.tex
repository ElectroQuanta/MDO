...
\begin{figure}[htb!]
\centering
    \includegraphics[width=1.0\columnwidth]{./img/flow-rs-parser.png}
...
\begin{figure}[htb!]
\centering
    \includegraphics[width=0.5\columnwidth]{./img/flow-rs-handle-query.png}
...
\begin{figure}[htb!]
\centering
    \includegraphics[width=0.6\columnwidth]{./img/flow-rs-handleCmd.png}
...
\paragraph{\textbf{UserDetection thread}}
...
\end{figure}
%
\paragraph{\textbf{FrameGrabber thread}}
...
\end{figure}
%
\paragraph{\textbf{LocalRx thread}}
...
\end{figure}
\paragraph{\textbf{LocalTx thread}}
...
\end{figure}
%
\paragraph{\textbf{AppParser thread}}
...
\end{figure}
% 
\paragraph{\textbf{CmdHandler thread}}
...
\end{figure}
\paragraph{\textbf{TwitterShare thread}}
...
\end{figure}
%
\paragraph{\textbf{FileTransfer thread}}
...
\end{figure}
%
\paragraph{\textbf{VidMan thread}}
Fig.~\ref{fig:flow-local-video-man} depicts the flowchart for the
\texttt{VidMan} thread. It checks for the current mode, and if it is the normal
one, it retrieves the current \texttt{Ad} to be played, if any. If there is an
\texttt{Ad} to be played a counter and a timer are initialized. While an
\texttt{User} is not detected and the timer is not elapsed, the videos are
fetched from a buffer and reproduced.
%
\begin{figure}[htb!]
\centering
    \includegraphics[width=0.42\columnwidth]{./img/flow-local-video-man.png}
  \caption{Flowchart: \texttt{Local System} --- \texttt{VidMan} thread}%
\label{fig:flow-local-video-man}
\end{figure}
\paragraph{\textbf{FragMan thread}}
Fig.~\ref{fig:flow-local-frag-man} depicts the flowchart for the
\texttt{FragMan} thread. It checks for the current mode, and if it is the normal
one, it retrieves the current \texttt{Ad} to be played, if any. If there is an
\texttt{Ad} to be played the on and off durations are calculated, based on the
\texttt{Ad} intensity, the timer \texttt{tim\_on} is initialized and the
fragrance diffusion is enabled. While an
\texttt{User} is not detected for the duration of the \texttt{Ad}, the fragrance
is diffused in intervals of \texttt{tim\_on} and \texttt{tim\_off}.
%
\begin{figure}[htb!]
\centering
    \includegraphics[width=0.4\columnwidth]{./img/flow-local-frag-man.png}
  \caption{Flowchart: \texttt{Local System} --- \texttt{FragMan} thread}%
\label{fig:flow-local-frag-man}
\end{figure}
%
\paragraph{\textbf{GIFGenerator thread}}
Fig.~\ref{fig:flow-local-gif-gen} depicts the flowchart for the
\texttt{GIFGenerator} thread. It waits for an event signaled by the \texttt{UI}
thread to start and then starts a timer. While the timer is not elapsed, the
camera frame is popped from the shared \gls{fifo} and stored in an internal
buffer. When the timer elapses, the \gls{gif} is stored in disk and this event
is signaled to the waiting thread --- the \texttt{UI} thread --- so it can
inform the user.
%
\begin{figure}[htb!]
\centering
...
\label{sec:cots}
For this project there are used several tools and libraries that are open-source
and helpful to develop all the subsystems.
