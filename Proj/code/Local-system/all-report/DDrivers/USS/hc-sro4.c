...
struct hc_sro4 {
	...
	//struct old_timeval32 time_triggered;
	//struct old_timeval32 time_echoed;
...
static irqreturn_t echo_received_irq(int irq, void *data)
{
	...
	int val;
	//struct old_timeval32 irq_tv;
	struct timespec64 irq_tv;
	//do_gettimeofday32(&irq_tv);
	ktime_get_ts64(&irq_tv);
...
static CLASS_ATTR_WO(sysfs_configure);
...
static struct attribute *hc_sro4_class_attrs[] = {
	&class_attr_sysfs_configure.attr,
  	NULL,
};
