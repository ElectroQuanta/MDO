...
void Ad::setMediaPath(const std::string mediaPath){
    pthread_mutex_lock(&_mutex);
    _mediaPath = QCoreApplication::applicationDirPath().toStdString() + "/" +
        ads_path_prefix + media_path_prefix + mediaPath;
    pthread_mutex_unlock(&_mutex);
}
...
bool Ad::save(){
    std::string path = QCoreApplication::applicationDirPath().toStdString() + "/" + ads_path_prefix + _fname + ".txt";
