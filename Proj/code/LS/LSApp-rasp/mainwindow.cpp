 ...
 */
#define SCREEN_RES_W 576 /**< Width in pixels */
#define SCREEN_RES_H 720 /**< Height in pixels */
#define SCENE_RES_H 500  /**< Height in pixels */
    ...
    , ui(new Ui::MainWindow)
{
    ...
    pthread_mutex_init(&_m_user_detect, NULL);
...
//    _ev_mode_check = new pEvent();
    ...
    QImage img = QImage((_path + QString(WELCOME_IMG_PATH)));
//    QImage img = QImage( WELCOME_IMG_PATH );
    ...
    if( !_face_cascade.load( _path.toStdString()
                             + std::string(FACE_CASCADE_FNAME) ) )
        updateStatusBar("ERROR: Could not load face detector");

    if( !_gesture_cascade.load( _path.toStdString()
                             + std::string(GESTURE_CASCADE_FNAME) ) )
        updateStatusBar("ERROR: Could not gesture detector");
...
// /                  &MainWindow::frag_diff_worker_thr, this );
    ...
    pthread_create( &_detect_user_thr, NULL,
                    &MainWindow::detect_user_worker_thr, this );
...
MainWindow::~MainWindow() {
    ...
    pthread_kill( _detect_user_thr, SIGKILL);
...
/**
 * @brief Detect user thread function
 * @param arg: ptr to a UI::MainWindow
 *
 * Reads from message queue and detects an user
 */
void* MainWindow::detect_user_worker_thr(void *arg){
    ...
    while(1){
        ...
        pthread_mutex_lock( &mw->_m_user_detect);
        mw->_user_detected = user_detected;
        pthread_mutex_unlock( &mw->_m_user_detect);
        
        user_detected = mw->detectUser();
...
#define TWITTER_API_FILE "../twitter/api.txt"
    ...
    std::ifstream apiFile ( _path.toStdString() +
                            std::string(TWITTER_API_FILE));
    if( apiFile.is_open() ){
        apiFile >> username;
        apiFile >> password;
        apiFile >> consumerKey;
...
#define CNT_RELOAD ((int)( PERIODIC_LIMIT_MS / MODE_PERIOD_MS ) )
    ...
    if (!cnt) {
      ...
      pthread_mutex_lock( &_m_user_detect);
      user_detected = _user_detected;
      pthread_mutex_unlock( &_m_user_detect);
...
//
//      if(cond < 0 ) /**< MSG queue not opened */
//      {
//          _mq_user_detect->Open(); /**< Try to open msg queue */
//          return false;
//      }
//      
//      if (cond == EAGAIN){
...
bool MainWindow::detectUser(){
   /**< Read Message Queue from daemon */
    ...
    //int cond = 0;
    //user_detected = cond;
      user_detected = _mq_user_detect->Receive();

      if(user_detected == -1 ) /**< MSG queue not opened */
      {
          _mq_user_detect->Open(); /**< Try to open msg queue */
          return false;
      }
      std::cout << "User detected: "
                << (user_detected - '0')
                //<< "User detected: " << user_detected
                //<< "; EAGAIN: " << EAGAIN
                //<< "; Ret value: " << (user_detected - '0')
